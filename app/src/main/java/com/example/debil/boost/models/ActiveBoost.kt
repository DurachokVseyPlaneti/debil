package com.example.debil.boost.models
import android.content.SharedPreferences
import com.example.debil.boost.AbsBoost
...
open class ActiveBoost: AbsBoost() {
    ...
    companion object {
        const val PRICE_COEFFICENT = 2.0
        const val INC_COEFFICENT = 1.5
        ...
        fun load(id: Int) : ActiveBoost {
            ...
            if (getted_title != null) {
                ...
                instance.level = shared?.getInt("boost_${id}_price", 0)!!
                return instance
            } else {
                return when (id) {
                    ...
                    4 -> DefaultActiveBoost4
                    5 -> DefaultActiveBoost5
    ...
    override fun buy() {
        apply()
        level++
        price = ceil(price * PRICE_COEFFICENT).toLong()
        inc = ceil(inc * INC_COEFFICENT).toLong()
        save()
    }
